  ...
  buildDays() {
      ...
      for (let i = monthBeforeNumberOfDays - dayOneWeekDay + 1; i <= monthBeforeNumberOfDays; i++) {
          this.days.push({'day': i, 'type': 'monthBefore', 'date':
                  `${monthBeforeDate.getFullYear()}-${this.getFormatedMonth(monthBeforeDate.getMonth() + 1)}-${this.getFormatedMonth(i)}`
          });
      }
      for (let i = 1; i <= dayOneNumberOfDays; i++) {
          this.days.push({'day': i, 'type': 'currentMonth',
              'date': `${this.date.getFullYear()}-${this.getFormatedMonth(this.month + 1)}-${this.getFormatedMonth(i)}`
          });
      }
      for (let i = 1; i < daysNextMonth; i++) {
          this.days.push({'day': i, 'type': 'monthAfter',
              'date': `${monthAfterDate.getFullYear()}-${this.getFormatedMonth(monthAfterDate.getMonth() + 1)}-${this.getFormatedMonth(i)}`
          });
      }
    }
